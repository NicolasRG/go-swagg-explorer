...
type Operation struct {
	...
	Parameters  []Parameter           `json:"parameters,omitempty" yaml:"parameters,omitempty"`   //open api 3 parameter type
	RequestBody *RequestBody          `json:"requestBody,omitempty" yaml:"requestBody,omitempty"` //TODO Change to empty interface
	Responses   map[string]Response   `json:"responses" yaml:"responses"`                         //TODO Change to empty interface
...
func AddToSwaggerAndRegister(docs Operation, config *SwaggerServerConfig, path string, passthrough http.HandlerFunc) {
	config.routeDocs = append(config.routeDocs, docs)
	fmt.Println(docs)
...
func GenerateDocs(config *SwaggerServerConfig) {
	...
	jsonData, _ := json.MarshalIndent(config.routeDocs, "", "  ")
